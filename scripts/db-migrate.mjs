...
async function ensureItemNpcColumn() {
  const columns = await prisma.$queryRawUnsafe(`PRAGMA table_info("Item")`);
  const hasNpcColumn = Array.isArray(columns)
    ? columns.some((column) => column?.name === "isSoldToNpc")
    : false;
...
try {
  ...
  console.log("SQLite migration applied successfully.");
} catch (error) {
  console.error("Failed to apply SQLite migration.");
  console.error(error);
  process.exitCode = 1;
} finally {
